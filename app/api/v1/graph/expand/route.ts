...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  let body: GraphExpandRequest
  try {
    body = (await request.json()) as GraphExpandRequest
  } catch {
    return jsonError(400, "invalid_json", "Request body must be valid JSON")
  }

  if (!body.entity_id?.trim()) {
    return jsonError(400, "invalid_request", "entity_id is required")
  }
  ...
  const graph = await buildGraphDeltaFromCenter({
    centerEntityId: body.entity_id.trim(),
    asOf,
    depth,
  ...
  if (!graph) {
    return jsonError(404, "entity_not_found", "Entity does not exist", {
